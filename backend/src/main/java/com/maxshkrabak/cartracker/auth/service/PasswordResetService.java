...
public class PasswordResetService {
    ...
    public PasswordResetToken verifyToken(String email, String token) {
        User user = userRepo.findByUsername(email).orElseThrow(InvalidResetTokenException::new);
        PasswordResetToken resetToken = tokenRepository.findByUserAndTokenHash(user, sha256(token.trim())).orElseThrow(InvalidResetTokenException::new);

        if (resetToken.getUsedAt() != null) { throw new InvalidResetTokenException(); }
        if (resetToken.getExpiresAt().isBefore(Instant.now())) { throw new InvalidResetTokenException(); }
    ...
    public void resetPassword(ResetPasswordRequest request) {
        PasswordResetToken resetToken = verifyToken(request.email(), request.token());
    ...
    public void requestPasswordReset(ForgotPasswordRequest request) {
        ...
        token.setExpiresAt(token.getCreatedAt().plusSeconds(600)); // 10 minute expiration
